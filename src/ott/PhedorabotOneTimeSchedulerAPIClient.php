...
final class PhedorabotOneTimeSchedulerAPIClient extends PhedorabotAPIClient{
  ...
  /** Subscription ID
  *
  * The one time service subscription as seen from your subscription page on
  * Phedorabot
  */
  ...
  /** Job ID
  *
  * Set a one time task job id already created on Phedorabot
  */
  ...
  /**
  * Task Name
  * The one time task name for easy identification
  */
  ...
  * Instant Execution Notification of your one time tasks. This should have
  * http protocols
  */
  ...
  /**
  * A description of this one time task, not more than 160 characters
  */
  ...
  /**
  * Add custom properties to this one time task, this will always be sent
  * back to your callback uri each time Phedorabot sends an instant
  * execution notification
  */
  public function addCustomProperty($key, $value){
    $properties = $this->getParameter('onetime_properties', array());

    if($key && strlen($key) && $value && strlen($value)){
      $properties[$key] = $value;
      $this->setParameter('onetime_properties', $properties);
    }
    return $this;
  }
  ...
  */
  public function addCustomHeader($key, $value){
    $headers = $this->getParameter('onetime_headers', array());

    if($key && strlen($key) && $value && strlen($value)){
      $headers[$key] = $value;
      $this->setParameter('onetime_headers', $headers);
    }
    return $this;
  }
   ...
   * this method, it will tell Phedorabot to use the current date and time at which
